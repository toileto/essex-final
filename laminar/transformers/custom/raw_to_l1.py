...
def raw_to_l1_envelope_encryption(data: dict, columns: list, extra_params: dict) -> dict:
    ...
    for column in columns:
        ...
        nonce = cipher.nonce
        ciphertext, tag = cipher.encrypt_and_digest(str(data[column]).encode())
        data[column] = b64encode(tag+nonce+ciphertext).decode()
    ...
    data["_wrapped_dek"] = b64encode(wrapped_dek).decode()
    return data
